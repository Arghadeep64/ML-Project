...
try:
    ...
    if st.session_state.show_results:
        ...
        if mood_choice != "All Songs":
            if mood_choice == "Sad": f_df = f_df[(f_df['Valence'] < 0.5) | (f_df['Acousticness'] > 0.6)]
            elif mood_choice == "Romantic": f_df = f_df[(f_df['Valence'] > 0.25) & (f_df['Valence'] < 0.75) & (f_df['Energy'] < 0.7)]
            elif mood_choice == "Happy/Energetic": f_df = f_df[(f_df['Valence'] > 0.55) | (f_df['Energy'] > 0.65)]

        if f_df.empty: st.warning("No matches found.")
        else:
