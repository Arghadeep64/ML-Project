...
# --- ULTRA-AESTHETIC IN-APP PLAYER UI ---
...
st.markdown("""
    ...
    /* 3. SLEEK SEARCH & MOOD UI */
    .stTextInput input { border-radius: 12px; border: 2px solid rgba(128,128,128,0.1); padding: 12px; background: rgba(255, 255, 255, 0.05) !important; }
    ...
    .footer-names {
        font-weight: 300; letter-spacing: 3px; text-transform: uppercase; font-size: 1.1rem;
...
if 'display_count' not in st.session_state:
    st.session_state.display_count = 10 # Reduced for smoother player loading

# --- HEADER ---
col_t, col_s = st.columns([2.5, 1.5])
with col_t:
    st.title("🎵 Music Recommendation System")
    st.markdown("<p style='opacity: 0.6;'>Your personalized soundscape generator</p>", unsafe_allow_html=True)
...
try:
        ...
        for i in range(show_now):
            ...
            st.markdown(f"""
                <div class="song-card">
                    ...
                    <div style="color: #1DB954; font-weight: 500; margin-bottom: 10px;">{row['ArtistName']}</div>
                </div>
            """, unsafe_allow_html=True)
            
            # THE "IN-APP" AUDIO PLAYER
            # This creates a Spotify window directly inside your card
    ...
    st.markdown(f"""
        <div class="footer-container">
            ...
            <p style="color: grey; font-size: 0.6rem; margin-top: 15px; opacity: 0.5;">© 2026 MoodTunes Music System</p>
        </div>
    """, unsafe_allow_html=True)
